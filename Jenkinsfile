        ...
        stage('Initialize the variables') {
            ...
            steps{
                script{
                    ...
                    bat """\"d:\\Git\\git-bash.exe\"  -i -c \"\"${AWSCLI}\" ec2 create-key-pair --key-name KeyExample --query 'KeyMaterial' --output text > key-example.pem\""""
                    bat """\"d:\\Git\\git-bash.exe\"  -i -c \"\"${AWSCLI}\" cloudformation create-stack --stack-name exampleStack --template-body file://scripts\\aws_deploy\\template.json --parameters ParameterKey=KeyExample,ParameterValue=KeyExample\""""
